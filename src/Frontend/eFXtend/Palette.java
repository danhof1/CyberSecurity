	...
	{		
		...
		for(int k = 0; k < scenes.size(); k++) //loops through scenes
		{
			...
			for(int i = 0; i < N; i++) //loops through objects in scene
			{
				...
				if(ProgressBar.class.isAssignableFrom(obj.getClass()))
				{
					ProgressBar bar = (ProgressBar)obj;

					String style = ("-fx-accent: GREEN; -fx-control-inner-background: WHITE; -fx-box-border: BLACK;");
					
					style = style.replace("GREEN", "#" + newPalette.getSecColor().toString().substring(2, 8));
					style = style.replace("WHITE", "#" + newPalette.getBgColor().toString().substring(2, 8));
					style.replace("BLACK", "#" + newPalette.getLineColor().toString().substring(2, 8));
					bar.setStyle(style);
				}
